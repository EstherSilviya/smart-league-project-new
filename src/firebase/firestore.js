...
export const getAchievementById = async (id) => {
  try {
    ...
    // achievements collection has no public read rule — silently return null
    return null;
  }
};
...
export const getNewsPostBySlug = async (slug) => {
  // Must include status filter — public users can only read published docs
  const q = query(collection(db, 'news'), where('postSlug', '==', slug), where('status', '==', 'published'));
  const snap = await getDocs(q);
  return !snap.empty ? { id: snap.docs[0].id, ...snap.docs[0].data() } : null;
};
